...
struct PredefinedModel: Identifiable, Hashable {
    ...
    let ramUsage: Double
    let hash: String
    let overrideDownloadURL: String?
    
    var downloadURL: String {
        if let overrideURL = overrideDownloadURL {
            return overrideURL
        }
...
enum PredefinedModels {
    static func getLanguageDictionary(isMultilingual: Bool, isLargeV3: Bool = false) -> [String: String]
    {
        if !isMultilingual {
            return ["en": "English"]
        } else if isLargeV3 {
    ...
    static let models: [PredefinedModel] = [
        ...
        PredefinedModel(
            name: "ggml-model",
            displayName: "Large v3 Turbo - ZH",
            size: "1.6 GiB",
            supportedLanguages: getLanguageDictionary(isMultilingual: true, isLargeV3: true),
            description: "Large model v3 Turbo, faster than v3 with similar accuracy, optimized for Chinese",
            speed: 0.7,
            accuracy: 0.96,
            ramUsage: 1.0,
            hash: "06309a34f03f8da6a0b726c5f1ad2f3afbcfe16a",
            overrideDownloadURL: "https://huggingface.co/BELLE-2/Belle-whisper-large-v3-turbo-zh-ggml/resolve/main/ggml-model.bin"
        ),
    ]
